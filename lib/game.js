...
class Game {
  ...
  drawGame() {
    ...
    const hitbox = this.canvas.height*.75 + this.len/2;
    ...
    this.song.notes.map(note => {
      ...
      if (note.y > hitbox + this.len) { note.y = 3000; }
      if (
        (note.x === this.canvas.width/4 && ((note.y < hitbox + this.len/2 && note.y > hitbox - this.len/2) && this.pressedRed === true && this.pressedBow)) ||
        (note.x === 3*this.canvas.width/8 && ((note.y < hitbox + this.len/2 && note.y > hitbox - this.len/2) && this.pressedYellow === true && this.pressedBow)) ||
        (note.x === this.canvas.width/2 && ((note.y < hitbox + this.len/2 && note.y > hitbox - this.len/2) && this.pressedGreen === true && this.pressedBow)) ||
        (note.x === 5*this.canvas.width/8 && ((note.y < hitbox + this.len/2 && note.y > hitbox - this.len/2) && this.pressedBlue === true && this.pressedBow))
      ) {
        this.score ++;
        this.resultIdx = 2;
        note.hit = true;
